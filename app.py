...
# ── Recover from browser back/forward "ghost page" (bfcache) ──────────────────
# Streamlit is a single-page app running over a websocket. When the user hits
# the browser Back/Forward button (most reliably reproduced in Safari), the
# browser can restore a frozen snapshot of the DOM from its back-forward cache
# instead of asking Streamlit to actually rerun — the websocket session behind
# that snapshot is dead, so parts of the UI that depend on a live session (our
# entire hand-built sidebar, since it's regenerated by Python on every run) can
# come back missing or stuck, even though static page content still shows.
# Forcing a real reload whenever a bfcache restore is detected (the standard
# `pageshow` + `event.persisted` signal) guarantees the user always lands on a
# fresh, fully-connected session — with the sidebar (and a way back to
# Overview) intact — instead of a half-alive ghost page with no way out.
components.html(
    """
    <script>
    try {
